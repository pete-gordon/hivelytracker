...
#warning API change.. hvl_load.* NEVER calls free on buf, since we do not know where it comes from
...
struct hvl_tune *hvl_load( const uint8 *buf, uint32 buflen, uint32 defstereo, uint32 freq )
{
  if( ( buf[0] == 'T' ) &&
      ( buf[1] == 'H' ) &&
      ( buf[2] == 'X' ) &&
...
//    voice->vc_PlantSquare  = 1;  /* hvl2wav had this line enabled */
      voice->vc_IgnoreSquare = 1;
      break;

    /* hvl2wav has case 0x5 and 0x03 with the exact same location */
...
void hvl_process_step( struct hvl_tune *ht, struct hvl_voice *voice )
{
  ...
  if( ( Instr ) && ( Instr <= ht->ht_InstrumentNr ) )
  {
    ...
    voice->vc_ADSR.aVolume     = Ins->ins_Envelope.aFrames ? Ins->ins_Envelope.aVolume*256/voice->vc_ADSR.aFrames : Ins->ins_Envelope.aVolume*256; // XXX
